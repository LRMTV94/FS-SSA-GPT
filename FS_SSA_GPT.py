# =====================================================================
#  FS-SSA, autoregressive: char-level GPT on tiny Shakespeare
...
#
#  Validation loss is evaluated every EVAL_INTERVAL steps (250) and the MINIMUM
#  is the reported number, with the iteration at which it occurs.
#  Tiny Shakespeare is 1.1M characters and this model has ~2.7M
#  parameters, so it overfits: the loss at the last step measures how much
#  was memorised, not how well the model generalises.
# =====================================================================
...
DROPOUT    = 0.2            # tiny Shakespeare overfits; nanoGPT uses 0.2 here

MAX_ITERS      = 10000
EVAL_INTERVAL  = 250
EVAL_ITERS     = 100        # batches averaged per evaluation
BATCH          = 64         # Batch dimension (64 --> 128)
...
CKPT_PATH    = "fsssa_gpt_state.json"
RESULTS_PATH = "results_fsssa_gpt.json"
PLOT_PATH    = "fsssa_gpt_curves.png"
SAMPLES_PATH = "fsssa_gpt_samples.txt"
...
print(f"tiny Shakespeare: {len(data):,} tokens | vocab {VOCAB} | train {len(train_data):,} | val {len(val_data):,}")


def get_batch(split, generator=None):
    d = train_data if split == "train" else val_data
    ix = torch.randint(len(d) - BLOCK - 1, (BATCH,), generator=generator)
...
def make_fs(K, width, threshold_scale, readout_scale, learnable, per_channel, n_channels):
    ...
    ''' Single place where the FS variant is chosen '''
...
def make_activation(kind, K, width, threshold_scale, readout_scale, learnable, per_channel, n_channels):
    ...
    '''Activation for the feed-forward branch'''

    if kind == "gelu":
        return nn.GELU()
    if kind == "fs":
        return make_fs(K, width, threshold_scale, readout_scale, learnable,
...
class CausalSpikingSelfAttention(nn.Module):
    ...
    Q and K are non-negative (unless signed we are used signed neurons), so
    every logit is >= 0 and the causal mask is a multiplication rather than
    a -inf fill. Rows are divided by the number of keys each position actually
    attends to, which grows with t: dividing by a constant would crush the
    beginning of the sequence.

    '''

    def __init__(self, d_model, n_heads, K, width, qk_scale, readout_scale, signed, learnable, dropout):
        super().__init__()
        assert d_model % n_heads == 0
        self.n_heads = n_heads
        self.d_head = d_model // n_heads
        ...
        def enc():
            if signed:
                return SignedFSNeuron(K, width, qk_scale, readout_scale, learnable,
                                      True, d_model)
...
def sanity_checks():
    ...
    x = torch.randn(4000, 8)
    print(f"  window at s={QK_SCALE:.3f} (K=2)   : [0, {hi:.3f}) "
          f"saturated {(x > hi).float().mean().item()*100:.1f}%")
    print("--- end checks ---\n")
...
def run(name, attention, activation, K, signed, learnable, seed):
    ...
    for it in range(MAX_ITERS + 1):
        ...
        if it % EVAL_INTERVAL == 0 or it == MAX_ITERS:
            ...
            print(f"  {name} s{seed} it {it:>5} | train {L['train']:.4f} | val {L['val']:.4f} (ppl {math.exp(L['val']):.2f}) | best {best['val']:.4f} @ {best['iter']} | {time.time()-t0:.0f}s", flush=True)
    ...
    return {"name": name, "seed": seed, "params": n_par,
...
if os.path.exists(CKPT_PATH):
    done = {f"{r['name']}|{r['seed']}": r for r in json.load(open(CKPT_PATH))}
    print(f"resuming: {len(done)} runs already done")

print("=" * 78)
print(f"tiny Shakespeare, char-level | {len(CONFIGS)} configs x {len(SEEDS)} seeds "
...
for name, attn, act, K, sg, ln in CONFIGS:
    for seed in SEEDS:
        ...
        json.dump(list(done.values()), open(CKPT_PATH, "w"), indent=1)
...
print("  (best, not final: 1.1M characters against ~2.7M parameters means the")
print("   model overfits, so the last value measures memorisation)")

print("=" * 90)
print(f"{'config':>18} | {'best val loss':>17} | {'ppl':>7} | {'@iter':>6} | {'final val':>9} | {'params':>9} | {'attn spk':>8} | {'min':>5}")
print("-" * 90)
...
for name, *_ in CONFIGS:
    ...
    print(f"{name:>18} | {bl:.4f} +/- {bs:.4f} | {math.exp(bl):7.2f} | {it:6.0f} | {fv:9.4f} | {par:>9,} | {('     --' if math.isnan(sa) else f'{sa:7.3f}')} | {mn:5.1f}")
...
print("\nWith 2 seeds these differences carry no usable interval: read them as a")
print("direction to confirm, not as a measurement.")
